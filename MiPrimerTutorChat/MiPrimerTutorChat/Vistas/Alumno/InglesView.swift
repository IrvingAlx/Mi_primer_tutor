...
struct InglesView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            if isLoading {
                cargarPreguntas(area: "ingles", numeroNivel: nivelActual)
            }
        }
    }
    ...
    func pasarAlSiguienteNivel() {
        ...
        guard let url = URL(string: "http://127.0.0.1:8000/preguntas_por_categoria?area=ingles&numero_nivel=\(nivelActual)") else {
            return
        }
